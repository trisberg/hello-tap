...
```
tanzu apps workload create hello-tap -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/hello-tap-source \
  --type web
```
...
```
URL=$(kubectl get services.serving.knative.dev/hello-tap -o=jsonpath='{.status.url}')
 ```
